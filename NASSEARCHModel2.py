...
from nas_model_search_2 import NASSEARCH
...
class NASSEARCHModel(nn.Module):
    ...
    def train_network(self, epoch, trainloader, valloader):
        self.train()
        # Update the learning rate based on the current epcoh
        self.scheduler.step(epoch - 1)
        index, top1, loss = 0, 0, 0
        lr = self.optim.param_groups[0]['lr']
        for num, (data, labels) in tqdm.tqdm(enumerate(trainloader, start=1), total=len(trainloader)):
            self.zero_grad()
            labels = torch.LongTensor(labels).to(self.device, non_blocking=True)
            data = data.to(self.device, non_blocking=True)

            # step architecture
            input_search, target_search = next(iter(valloader))
