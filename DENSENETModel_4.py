...
from model_densenet_4 import DenseNet
from tools import *
...
class DENSENETModel(nn.Module):
    def __init__(self, lr, lr_decay, C, n_class, m, s, test_step, pooling_mode, **kwargs):
        super(DENSENETModel, self).__init__()
        # Densenet
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.speaker_encoder = DenseNet(pooling_mode=pooling_mode).to(self.device)
        # Classifier
...
class DENSENETModelMulti(nn.Module):
    ...
    def train_network(self, epoch, loader):
        ...
        for num, batch in tqdm.tqdm(enumerate(loader, start=1), total=len(loader)):
            ...
            for idx_loss, speaker_loss_ in self.speaker_loss.items():
                ...
                total_loss += nloss / len(self.speaker_loss)  # Accumulate loss
...
class DENSENETModelMulti2(nn.Module):
    def __init__(self, lr, lr_decay, C, n_class, m, s, test_step, **kwargs):
